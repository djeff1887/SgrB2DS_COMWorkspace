...
if source=='DSIX':
    ...
    innerradius=1
    outerradius=len(radialdensitylist)-30
...
romans={'SgrB2S':'Sgr B2(S)','DSi':'DS1','DSii':'DS2','DSiii':'DS3','DSiv':'DS4',
        'DSv':'DS5','DSVI':'DS6','DSVII':'DS7','DSVIII':'DS8','DSIX':'DS9'}
...
textheight={'DSi':95,'DSii':275,'DSiii':425,'DSiv':190,'DSVI':95,'DSVII':45,'DSVIII':95,'DSIX':45}
if source in textheight.keys():
    h=textheight[source]
else:
    h=avgtexlist[0]+175
...
if source == 'DSiv':
    ...
    ax0.text(0,h,romans[source],size=12,color='black')
    ...
    ax0.set_ylim(ymax=plottexmax+50,ymin=150)#(max(upperfill)+30))
    ax0.legend()
    ax0.tick_params(direction='in')
    ax0.tick_params(axis='x',labelcolor='w')
    ax1.tick_params(axis='x',top=True,direction='in')
    figsavepath=figpath+f'_{dataversion}_radialavgtex_r{r}px_rphys{int(pixtophysicalsize.value)}AU_smoothed.png'
    ...
    ax0.plot(copy_centrtopix,fit_spl(copy_centrtopix),color='orange',label=f'$\\alpha$={round(fit_spl.alpha.value,2)} \u00B1 {round(splperr[2],2)}',zorder=1)
    ax0.plot(copy_centrtopix,fit_pl(copy_centrtopix),color='red',ls='-',zorder=2,label=f'$\\alpha_1$={round(fit_pl.alpha_1.value,2)} \u00B1 {round(perr[2],2)}\n$\\alpha_2$={round(fit_pl.alpha_2.value,2)} \u00B1 {round(perr[3],2)}\n$r_{{break}}$={round(fit_pl.x_break.value)} \u00B1 {round(perr[1])}')#[1:] for ds7
    ax0.text(0,h,romans[source],size=12,color='black')#annotate(romans[source],xy=(0,0),xytext=((1000,(avgtexlist[0]+20))))
    ...
    ax0.tick_params(axis='x',labelcolor='w')
    ax1.tick_params(axis='x',top=True,direction='in')
    figsavepath=figpath+f'_{dataversion}_radialavgtex_LMLSQ_r{r}px_rphys{int(pixtophysicalsize.value)}AU_smoothed.png'
