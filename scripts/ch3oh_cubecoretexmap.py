...
catdir_ch3oh=catdir[catdir['tag'] == 32504]
catdir_qrot300=10**catdir_ch3oh['lg(Q(300))']

Cfreqs, Caij, Cdeg, CEU, qrot = get_molecular_parameters('CH3OH, vt=0-2', catalog='CDMS', fmin=150*u.GHz, fmax=300*u.GHz)
...
trotdict={'SgrB2S':300*u.K,'DSi':400*u.K,'DSii':200*u.K,'DSiii':300*u.K,'DSiv':150*u.K,'DSv':150*u.K,'DSVI':425*u.K,'DSVII':200*u.K,'DSVIII':175*u.K,'DSIX':150*u.K,'DS10':150*u.K,'DS11':100*u.K,'DSX':100*u.K}#old dsv - 1e16, 150K
testT=trotdict[source]#500*u.K
ntotdict={'SgrB2S':1e18*u.cm**-2,'DSi':1e18*u.cm**-2,'DSii':1e17*u.cm**-2,'DSiii':1e18*u.cm**-2,'DSiv':1e18*u.cm**-2,'DSv':1e17*u.cm**-2,'DSVI':3e18*u.cm**-2,'DSVII':1e17*u.cm**-2,'DSVIII':1e17*u.cm**-2,'DSIX':1e17*u.cm**-2,'DS10':1e16*u.cm**-2,'DS11':1e16*u.cm**-2,'DSX':1e15*u.cm**-2}#priors for Jeff+2024: {'SgrB2S':1e18*u.cm**-2,'DSi':1e18*u.cm**-2,'DSii':1e17*u.cm**-2,'DSiii':1e17*u.cm**-2,'DSiv':1e18*u.cm**-2,'DSv':1e17*u.cm**-2,'DSVI':1e18*u.cm**-2,'DSVII':1e17*u.cm**-2,'DSVIII':1e17*u.cm**-2,'DSIX':1e17*u.cm**-2,'DS10':1e16*u.cm**-2,'DS11':1e16*u.cm**-2,'DSX':1e15*u.cm**-2}
    ...
    for key in dictkeys:
        ...
        for i in range(len(temptransdictkeys)):
            if 'restfreq' in temptransdictkeys[i]:
                ...
                temp=velflux_T/linewidth#***What's going on here? Should this be fixed because it uses one uniform linewidth
    ...
    for i in range(iterations):
        ...
        if os.path.isfile(maskedmom0fn):
            ...
            if os.path.isfile(peakintfn):
                ...
                slab=slab.with_spectral_unit((u.km/u.s),velocity_convention='radio',rest_value=line)
            ...
            if quantum_numbers[i] == '10(6,4)-10(7,3)':
                pdb.set_trace()
            slab=slab.with_spectral_unit((u.km/u.s),velocity_convention='radio',rest_value=line)#spwrestfreq)
            if transition in excludedlines[source]:
                ...
                sigma1mom0=stdcutout.data*line_width
                ...
                slabfwhm=slab.linewidth_fwhm()#(7*u.MHz/line)*c.to('km s-1')#
                cubemask=(slabspecax[:,None,None] < (velocityfield_representative + fwhm_representative)[None,:,:]) & (slabspecax[:,None,None] > (velocityfield_representative - fwhm_representative)[None,:,:])
                oldstyleslab=oldstyleslab.with_spectral_unit((u.km/u.s),velocity_convention='radio',rest_value=line)
                #if imgnum > 0:
                #    pdb.set_trace()
...
sourcelocs={'SgrB2S':'/sep2023-5removelasttorsionalline/','DSi':'/sep2023-5addvt2linesbackin/','DSii':'/sep2023-2widerrefslab/','DSiii':'/sep2024_1_testingnewch3ohprediction/','DSiv':'/sep2023-4nextinline/','DSv':f'/sep2023phi_nu&doublet/','DSVI':'/sep2024_1_testingnewch3ohprediction/','DSVII':f'/sep2023phi_nu&doublet/','DSVIII':f'/sep2023phi_nu&doublet/','DSIX':f'/sep2023phi_nu&doublet/'}#,'DS10':'/march2023discovery_5kmslw/','DS11':f'/march2023discovery_5kmslw_{int(testT.value)}K/','DSX':f'/Kfield7originals_{int(testT.value)}K_trial1_noexclusions/'}#'/Kfield10originals_trial7_field10errors_newexclusion_matchslabwidthtorep/'
...
for imgnum in range(len(datacubes)):
    ...
    maintable = CDMS.query_lines(min_frequency=freq_min,max_frequency=freq_max,min_strength=-500,molecule='032504 CH3OH, vt=0-2',get_query_payload=False)
                                    
    print('Gathering Splatalogue table parameters')
    nus=maintable['FREQ']#Rest frequencies
