...
sourcedict={'SgrB2S':'/sep2023-5removelasttorsionalline/','DSi':'chisquare_goodnessoffit_4_3contamsremoved/','DSii':'/sep2023-2widerrefslab/','DSiii':'/sep2023-3vt2doublet/','DSiv':'/sep2023-4nextinline/','DSv':f'/sep2023phi_nu&doublet/','DSVI':'/sep2023-2removenewvt1line/','DSVII':f'/sep2023phi_nu&doublet/','DSVIII':f'/sep2023phi_nu&doublet/','DSIX':f'/aug2023qrotfix/'}
pixdict={'SgrB2S':(70,59),'DSi':(36,40),'DSii':(22,24),'DSiii':(24,24),'DSiv':(32,31),'DSv':(19,19),'DSVI':(62,62),'DSVII':(75,75),'DSVIII':(50,50),'DSIX':(34,35)}
pixel=pixdict[source]
sourcepath=sourcedict[source]
origsourcepath=f'/blue/adamginsburg/d.jeff/SgrB2DSreorg/field{fnum}/C2H5OH/{source}/{sourcepath}'#origsourcedict[source]
...
home=origsourcepath
rotdiagpath=reorgpath+'pixelwiserotationaldiagrams/'

debuglabel='naturallog'#sourcepath.replace('/','')
...
infile=open(origsourcepath+'C2H5OHlinesdict.obj','rb')
spwdict=pickle.load(infile)

fulltexmap=fits.getdata(reorgpath+'texmap_3sigma_allspw_withnans_weighted.fits')
...
testT=fulltexmap[pixel[0],pixel[1]]#trotdict[source]
...
allmaster=QTable.read(home+'mastereuksqnsfreqsdegens.fits')
mastereuks=allmaster['Eupper']
masterdegens=allmaster['Degeneracy']
masterqns=allmaster['QNs']
masterfreqs=allmaster['Reference Frequency']*u.MHz
...
for px in pixellist:
    ...
    for z in range(testzshape):
        if nugsmap[z,y,x] <= 0 or np.isnan(nugsmap[z,y,x]):
            ...
            freqsfitted.append(masterfreqs[z].value)
    if len(nupperstofit)==0:
        ...
        fit_lin=fit(linemod,eukstofit,np.log(nupperstofit), weights=errstofit)#10
        obsTrot=-1/(fit_lin.slope)#np.log10(np.e)
        qrot_partfunc=qrot(obsTrot*u.K)*u.dimensionless_unscaled#9473.271845042498*u.dimensionless_unscaled
        
        bslist=[]
        for nug,euk,weight,var in zip(np.log(nupperstofit),eukstofit,errstofit,log10variances):#10
        ...
        for line in bootlines:
            tempbootTrot=-1/(line.slope)#np.log10(np.e)
            tempbootslope=line.slope.value
            tempbootNtot=qrot_partfunc*np.exp(line.intercept)
        ...
        bootNstd=(qrot_partfunc*np.exp(interimfactor)).value
        ...
        altNtot=qrot_partfunc*np.exp(fit_lin.intercept)
        print(f'Alt Ntot: {altNtot}')
        obsNtot=altNtot
        
        upper=fit_lin.intercept.value+int_bootNstd
        lower=fit_lin.intercept.value-int_bootNstd
        upperNtot=qrot_partfunc*np.exp(upper)
        lowerNtot=qrot_partfunc*np.exp(lower)
        ...
        dobsNtot=np.sqrt((qrot_partfunc*np.exp(fit_lin.intercept)*b_unc)**2)*u.cm**-2#*np.log(10)
        ...
        dots=plt.errorbar(eukstofit,np.log(nupperstofit),yerr=log10nuerr,fmt='o')
        for eupper,nupper,qn,num in zip(eukstofit,nupperstofit,qnsfitted,np.arange(len(qnsfitted))):
            txt=plt.text(eupper,np.log(nupper),(num+1),fontsize=7,label=qn)
            txt.set_path_effects([pe.withStroke(linewidth=1,foreground='w')])
        qnlegend=QTable([(np.arange(len(qnsfitted))+1),qnsfitted,freqsfitted,eukstofit],units=['','','MHz','K'])
        print(qnlegend)
        ...
        qnlegend.write(rotdiagpath+f'{debuglabel}_qnlegend_{y}_{x}.fits',overwrite=True)
        plt.show()
